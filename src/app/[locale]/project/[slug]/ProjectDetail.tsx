    ...
    <div ref={inViewRef} className="w-screen h-screen bg-black text-[#ecebe8] selection:bg-[#dbd9d3] selection:text-black overflow-hidden">
      <CustomCursor>
        ...
        <div className="h-full p-3 xxs:p-4 xl:p-6 2xl:p-8 flex flex-col">
          ...
          <article className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 min-h-0">
            {/* Image Column */}
              ...
              <div className="border-l-4 border-[#dbd9d3] pl-4 h-full flex items-center">
                {project.gallery && project.gallery.length > 0 ? (
                    ...
                    {project.gallery.map((image, index) => (
                      <SwiperSlide key={index}>
                        <div className="w-full h-full flex items-center justify-center bg-gray-900/30">
                          <Image src={image.src} alt={`${project.name} - Screenshot ${index + 1}`} width={image.width} height={image.height} className="w-full h-full object-cover object-center" priority={index === 0} />
                        </div>
                      </SwiperSlide>
                    ))}
                  </Swiper>
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-gray-900/30 rounded overflow-hidden">
                    <Image src={project.image.src} alt={`${project.name} - Project preview`} width={project.image.width} height={project.image.height} className="w-full h-full object-cover object-center" priority />
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Content Column */}
            <div className="order-2 lg:order-2 flex flex-col gap-4 lg:gap-6 overflow-y-auto relative" ref={contentColumnRef} onScroll={handleContentScroll}>
              {/* Header */}
              ...
              {/* Description */}
              <section aria-labelledby="overview-title" className="space-y-3 flex-shrink-0">
                <div className="relative inline-block">
                  <h2 id="overview-title" ref={overviewTitleRef} className="text-base xxs:text-lg sm:text-xl lg:text-2xl font-bold uppercase tracking-wide">
                    {t('overview')}
                  </h2>
              ...
              <section aria-labelledby="tech-title" className="space-y-3">
                <div className="relative inline-block">
                  <h2 id="tech-title" ref={techTitleRef} className="text-lg xxs:text-xl font-bold uppercase tracking-wide">
                    {t('techStack')}
                  </h2>
                  <span className="title-underline border-b-2 border-[#dbd9d3] absolute bottom-0 left-0 inline-block" />
                </div>
                <div ref={techItemsRef} className="grid grid-cols-2 gap-2" role="list" aria-label="Technologies used">
                  {project.techStack.map((tech, index) => (
                    <div key={index} className="flex items-center gap-2" role="listitem">
                      <span className="text-[#dbd9d3]" aria-hidden="true">|</span>
              ...
              {/* Links */}
              <section aria-labelledby="links-title" className="space-y-3 flex-shrink-0">
                <div ref={linksDividerRef} className="border-t border-[#a9a69e] pt-4"></div>
                <div className="relative inline-block">
                  <h2 id="links-title" ref={linksTitleRef} className="text-base xxs:text-lg sm:text-xl lg:text-2xl font-bold uppercase tracking-wide">
                    {t('links')}
                  </h2>
                  <span className="title-underline border-b-2 border-[#dbd9d3] absolute bottom-0 left-0 inline-block" />
                </div>
                <div ref={linksRef} className="space-y-2">
                  <a href={project.repository} target="_blank" rel="noopener noreferrer" className="relative block text-sm xxs:text-base sm:text-base lg:text-lg uppercase pb-1 cursor-pointer" aria-label={`View source code for ${project.name} on GitHub`}>
                    <span className="relative inline-flex items-center gap-2 after:content-[''] after:transition-transform after:duration-500 after:h-[1px] after:w-full after:left-0 after:bottom-0 after:bg-[#ecebe8] after:absolute after:scale-x-0 after:scale-y-100 after:origin-[0%_100%] hover:after:scale-x-100">
                      <span className="text-[#dbd9d3]" aria-hidden="true">▸</span>
                      {t('sourceCode')}
                      <RxArrowTopRight className="text-[#dbd9d3]" aria-hidden="true" />
                    </span>
                  </a>
                  {project.deploy && (
                    <a href={project.deploy} target="_blank" rel="noopener noreferrer" className="relative block text-sm xxs:text-base sm:text-base lg:text-lg uppercase pb-1 cursor-pointer" aria-label={`View live demo of ${project.name}`}>
                      <span className="relative inline-flex items-center gap-2 after:content-[''] after:transition-transform after:duration-500 after:h-[1px] after:w-full after:left-0 after:bottom-0 after:bg-[#ecebe8] after:absolute after:scale-x-0 after:scale-y-100 after:origin-[0%_100%] hover:after:scale-x-100">
                        <span className="text-[#dbd9d3]" aria-hidden="true">▸</span>
                        {t('liveDemo')}
                        <RxArrowTopRight className="text-[#dbd9d3]" aria-hidden="true" />
